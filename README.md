...
normal declarations, unlayered consumer CSS automatically takes priority over
mine.css regardless of selector specificity or stylesheet order:

```css
@import 'mine.css';

/* This unlayered site rule wins over the defaults in @layer mine. */
h1 { font-weight: 700; }
```

Sites that organize all their CSS into layers should declare an explicit order
with `mine` first:
...
Import the package root normally; adding `layer(mine)` to that import would
unnecessarily nest the stylesheet's existing layer. Optional layout and top-bar
stylesheets retain their existing unlayered behavior when linked directly. A
CSS entry point can instead collect optional styles in a distinct `sidecar`
layer between the framework defaults and application styles:
...
The same pattern works for color and syntax-theme sidecars. Keeping `sidecar`
...
Responsive audio and canvas defaults draw on the practical remedies collected in [CSS Remedy][css-remedy].
